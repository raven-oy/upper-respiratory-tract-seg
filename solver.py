...
class Solver(object):
	...
	def train(self):
		...
		if os.path.isfile(unet_path):
			...
			# If no best model saved, save the last one
			if best_unet is None:
				best_unet = self.unet.state_dict()
				torch.save(best_unet, unet_path)
	...
	def test(self):
		...
		if os.path.isfile(unet_path):
			...
			print('No trained model found at %s' % unet_path)
			return
		...
		f = open(os.path.join(self.result_path,'result.csv'), 'a', encoding='utf-8', newline='')
		wr = csv.writer(f)
		wr.writerow([self.model_type,acc,SE,SP,PC,F1,JS,DC,self.lr,0,self.num_epochs,self.num_epochs_decay,self.augmentation_prob])
		f.close()

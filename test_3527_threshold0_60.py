...
# output dirs
out_dir = os.path.join(config.result_path, config.model_type + '_3527_threshold0.60')
vis_dir = os.path.join(out_dir, 'vis')
os.makedirs(vis_dir, exist_ok=True)

# per-image csv
per_image_csv = os.path.join(out_dir, 'per_image_counts_thr0.60.csv')
summary_csv = os.path.join(out_dir, 'summary_thr0.60.csv')
...
for i, (images, GT) in enumerate(test_loader):
    ...
    # metrics using evaluation functions (they accept threshold param)
    a = get_accuracy(SR,GT,threshold=threshold)
    s = get_sensitivity(SR,GT,threshold=threshold)
    sp = get_specificity(SR,GT,threshold=threshold)
    ...
    # per-image counts (binary at threshold)
    SR_mask = (SR > threshold).float()
    ...
    vutils.save_image(SR_mask.data.cpu(), os.path.join(vis_dir, f'vis_{i:03d}_SR_mask_thr{int(threshold*100):03d}.png'))
    ...
    blended.save(os.path.join(vis_dir, f'vis_{i:03d}_overlay_thr{int(threshold*100):03d}.png'))
...
with open(summary_csv, 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['model', 'acc','SE','SP','PC','F1','IoU','Dice','image_size','n_samples','threshold'])
    writer.writerow([os.path.basename(model_path), acc,SE,SP,PC,F1,JS,DC,image_size,length,threshold])
